...
%1. amp2dB function for converting normalized amplitude to decibels relative to full scale (dBFS)
...
%   yin is a popular F0 estimation algorithm developed by Alain de Cheveigne and Hideki Kawahara
%   Their paper on yin, published in JASA, can be downloaded here: audition.ens.fr/adc/pdf/2002_JASA_YIN.pdf
...
%.SN - %average of attack window and sustain window normalized RMS amplitudes (i.e, average of .Attack_RMS_NormAmp and .Sustain_RMS_NormAmp). Used for deciding onset when there is an ISI conflict (not printed to text file).
...
%the rms across the sustainWindow (including the current sample) is calculated. If sustainWindow - noiseWindow is greater than or equal to SNdiff, then 
...
%time point is recorded as an onset.  However, if there is an ISI conflict, then the average of the attackWindow rms and sustainWindow rms is 
...
%of average attack/sustain rms places somewhat more weight on the ramping up of the onset over the attackWindow (i.e., the slope and amplitude of the initial onset ramp)compared 
%to the sustain of the onset over the sustainWindow.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%Params used for all Narr files
%  noiseWindow = 100;
%     attackWindow = 15;
%     sustainWindow = 75;
%     maxNoise = 0.02;
...
function OnsetInfo = AcousticOnsetsNormAmp(wav,samplerate,noiseWindow,attackWindow,sustainWindow,maxNoise,SNdiff,ISI,filename)                          
...
for part = 1:length(wavfns)
 ...
 %% DISPLAY INITIALIZATION
    ...
    for i = sampStartTime:sampEndTime
        ...
        %dBNoise = amp2dB(aveNoise); %convert value to dB
        if aveNoise <= maxNoise %fits criterion for noise floor, so now check on attack criterion
            ...
            %dBAttack = amp2dB(attackSignal); %convert value to dB
            if attackSignal - aveNoise >= SNdiff %fits criterion for attack SNdiff, so now check on sustain criterion
                ...
                %dBSustain = amp2dB(sustainSignal); %convert value to dB
                if sustainSignal - aveNoise >= SNdiff %fits criterion for sustain SNdiff, now consider as a candidate and check ISI
                    ...
                    elseif i - ONSET.OnsetTime_SMP(uBound) >= (sampAttackWindow + sampSustainWindow) %if candidate is outside the range of the previous onset found %If this is not the first onset you have found, make sure this onset doesn't conflict with 
                        %check previous onset based on the minimum ISI you specified
        ...
        if onsetTrip == 1 %a legitimate onset has been found, enter it into the arrays      
            ...
            ONSET.Noise_RMS_DBFS(uBound, :) = amp2dB(ONSET.Noise_RMS_NormAmp(uBound));  %normalized RMS amplitude over the noise window
            ONSET.Attack_RMS_DBFS(uBound, :) = amp2dB(ONSET.Attack_RMS_NormAmp(uBound)); %normalized RMS amplitude over the noise window
            ONSET.Sustain_RMS_DBFS(uBound, :) = amp2dB(ONSET.Sustain_RMS_NormAmp(uBound)); %normalized RMS amplitude over the noise window
            ONSET.Onset_RMS_DBFS(uBound, :) = amp2dB(ONSET.Onset_RMS_NormAmp(uBound)); %normalized RMS amplitude over the noise window
